 ...
 * description：
 */
class WanAndroidCommonItemViewHolder(itemView: View) : BaseViewHolder(itemView) {
    ...
    companion object {
        val viewHolderFactory = object : IViewHolderFactory<BaseViewHolder> {
            override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder {
                return WanAndroidCommonItemViewHolder(LayoutInflater.from(parent.context).inflate(R.layout.item_home_common, parent, false))
            }

            override fun <T : Any?> getItemViewType(position: Int, data: T): Int {
                return WanAndroidListItemType.TYPE_COMMON
            }
        }
    }
